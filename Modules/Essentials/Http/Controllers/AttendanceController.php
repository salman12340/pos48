...
class AttendanceController extends Controller
{
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($attendance)
                    ...
                    ->editColumn('clock_in', function($row) {
                        ...
                        if (!empty($row->clock_in_meter)) {
                            $html .= '<br>' . $row->clock_in_meter .'<br>';
                        }
                    ...
                    ->editColumn('clock_out', function($row) {
                        ...
                        if (!empty($row->clock_out_meter)) {
                            $html .= '<br>' . $row->clock_out_meter .'<br>';
                        }
                ...
                foreach ($attendance as $user_id => $value) {
                    ...
                    EssentialsAttendance::updateOrCreate(
                        ...
                        [
                            ...
                            'clock_in_meter' => $value['clock_in_meter'],
                            'clock_out_meter' => $value['clock_out_meter'],
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            $input = $request->only(['clock_in_time', 'clock_out_time', 'ip_address', 'clock_in_note', 'clock_out_note', 'clock_in_meter', 'clock_out_meter']);
    ...
    public function clockInClockOut(Request $request)
    {
        ...
        try {
            ...
            if ($type == 'clock_in') {
                $document = $this->moduleUtil->uploadFile($request, 'document', 'documents');
                $data = [
                    'business_id' => $business_id,
                    'user_id' => auth()->user()->id,
                    'clock_in_time' => \Carbon::now(),
                    'clock_in_note' => $request->input('clock_in_note'),
                    'clock_in_meter' => $request->input('clock_in_meter'),
                    'ip_address' => $this->moduleUtil->getUserIpAddr(),
                ...
                $data = [
                    ...
                    'clock_out_meter' => $request->input('clock_out_meter'),
        ...
        try {
            ...
            if ($request->hasFile('attendance')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    $temp['clock_in_meter'] = !empty($value[6]) ? trim($value[6]) : null;
                    $temp['clock_out_meter'] = !empty($value[7]) ? trim($value[7]) : null;
